...
class EPDMooncakeConnectorWorker(UpstreamMooncakeConnectorWorker):
        ...
        self.max_transfer_descriptors = max(
            ...
            int(
                extra.get(
                    ...
                    os.getenv("MOONCAKE_EPD_MAX_TRANSFER_DESCRIPTORS", 32),
                )
                or 32
            ),
        )
    ...
    def _publish_connector_metrics(self) -> None:
        ...
        if (pending is None or pending.is_empty()) and not has_path_pending:
            return
        sink.record(pending, path_totals=pending_by_path)
        self._connector_metrics_pending = LayeredTransferWorkerMeta()
        self._connector_metrics_pending_by_path = {}
                ...
                for group_local_block_id, group_remote_block_id in zip(
                    ...
                    if can_coalesce:
                        ...
                        for local_block_id, remote_block_id in zip(
                            ...
                            descriptor_paths.append(routing_path)
                            path_descs += 1
                            path_bytes += transfer_len

            if path_descs > 0 or path_bytes > 0:
                prev_descs, prev_bytes = path_stats.get(routing_path, (0, 0))
                path_stats[routing_path] = (
                    prev_descs + path_descs,
                    prev_bytes + path_bytes,
                )
    ...
    def _send_blocks(
        ...
        if (not self.layered_kv_transfer or len(src_ptrs) <= 1) and not must_chunk_for_transport:
        ...
        descriptors_per_group = infer_descriptors_per_group(
            len(src_ptrs),
            total_regions=self._registered_region_count,
            layers_per_group=self.layers_per_group,
        )
        descriptors_per_group = min(descriptors_per_group, single_batch_desc_limit)
    ...
    def _send_region_group_dispatch(
        ...
        *,
        ...
        if dispatch.ret_code == 0 and self.group_delay_ms > 0:
            self._accumulate_worker_meta(
                LayeredTransferWorkerMeta(
                    accumulated_group_delay_ms=self.group_delay_ms
                )
